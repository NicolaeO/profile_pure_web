const chirurgie_4 = `
---

# CHIRURGIE   ENDOCRINĂ

- question: 'În legatura cu guşa sporadică sunt adevărate următoarele afirmaţii:'
  answers:
  - c. Apare la cca 20% din populaţia unei zone
  correct:
  - a. Creştere de volum a tiroidei
  - b. Normofuncţie tiroidiană
  - d. Apare mai frecvent la femei
  - e. Se transmite autosomal recesiv
  Răspuns corect:  A, B, D, E

- question: 'Convulsiile, simptomele Chvostek şi Trusso după strumectomie se datorează:'
  answers:
  - a. Hipotiriozei
  - b. Crizei  tiriotoxice
  - c. Traumatisme ale nervilor laringieni
  - e. Efectelor reziduale ale tirotoxicozei
  correct:
  - d. Hipoparatiriozei
  Răspuns corect:  D

- question: 'Indicaţii pentru tratamentul chirurgical al unei guşi sunt:'
  answers:
  - b. Guşa parenchimatoasă difuză la debut
  correct:
  - a. Eşecul tratamentului medical
  - c. Guşa hipertiroidizată
  - d. Guşa nodulară
  - e. Guşa suspectă de malignizare
  Răspuns corect:  A, C, D, E

- question: 'În funcţie de situaţie pot fi utilizate următoarele tipuri de intervenţii chirurgicale în tratamentul unei guşi:'
  answers:
  - e. Disectie cervicala radicala (radical neck dissection)
  correct:
  - a. Tiroidectomia subtotală
  - b. Lobectomia totală
  - c. Tiroidectomia totală
  - d. Lobectomia subtotală
  Răspuns corect:  A, B, C, D

- question: 'Imaginea scintigrafică a adenomului toxic arată:'
  answers:
  - a. Captare difuză şi omogenă a trasorului radioactiv
  - b. Captare difuză dar neomogenă a trasorului radioactiv
  - c. Captare în “tablă de şah”
  - e. „Nodul rece”
  correct:
  - d. “Nodul fierbinte”
  Răspuns corect:  D

- question: 'Tratamentul cu iod radioactiv în tireotoxicoze are următoarele indicaţii:'
  answers:
  - a. Boala Basedow
  - b. Adenomul toxic
  - c. Asocierea hipertiroide – cancer tiroidian
  - e. Hipertioridii secundare tumorilor secundare tireotrope
  correct:
  - d. Tirotoxicozele induse de administrarea de hormoni tiroidieni
  Răspuns corect:  D

- question: 'Complicaţiile tiroidectomii includ următoarele, cu excepţie:'
  answers:
  - a. Pareza de recurent
  - b. Mixedem
  - c. Criză tetanică
  - d. Criza tirotoxică
  correct:
  - e. Sindrom exoflatmic
  Răspuns corect:  E

- question: 'Cea mai frecventa complicaţie a strumectomiei este:'
  answers:
  - a. Lezarea nervului recurent
  - b. Recidiva tiriotoxicozei
  - c. Hiperplazia aparatului limfoid
  - d. Tetania
  correct:
  - e. Hipotirioza
  Răspuns corect:  E

- question: 'Adevărate pentru neoplazia endocrină multiplă de tip I (MEN1) sunt următoarele afirmaţii:'
  answers:
  - b. Este compusă din tumori sau hiperplazii ale suprarenalelor, pancreasului şi hipofizei
  correct:
  - a. Este compusă din tumori sau hiperplazii ale paratiroidelor, insulelor pancreatice şi hipofizei
  - c. În cadrul MEN 1 hiperparatiroidismul este cea mai comună manifestare (90-100%)
  - d. Adenomul hipofizar pare a fi prezent la aproape toţi pacienţii cu MEN 1
  - e. MEN 1 este o anomalie genetică autozomal dominantă
  Răspuns corect:  A, C, D, E

- question: 'Ce afirmaţie referitor la tiroida cronică Riedel este falsă?'
  answers:
  - a. Glanda tiroidă este invadată de ţesut fibros sclerozant, dens
  - b. Poate însoţi alte afecţiuni fibrozante idiopatice
  - c. Clinic apare ca o guşă dură, lemnoasă
  - e. Scintigrama cu I131 evidenţiază o captare difuză, neomogenă
  correct:
  - d. Pacienţii prezentă guşă omogenă (nodulară bine deliminată, palpator moale)
  Răspuns corect:  D

- question: 'Ce afirmaţii referitor la tiroidită limfocitară Hashimoto sunt adevărate?'
  answers:
  - e. Clinic apare ca o guşă dură, lemnoasă, compresivă
  correct:
  - a. Este boala autoimună
  - b. Apare predominant la femei
  - c. În serul pacienţilor se evidenţiază titruri mari de anticorpi antitiroidieni
  - d. Boala este deseori familială însoţită cu alte afecţiuni autoimune
  Răspuns corect:  A, B, C, D

- question: 'O complicaţie care nu este caracteristică interventiilor chirurgicale pe glanda tiroidă este:'
  answers:
  - a. Hemoragie
  - b. Embolismul aerian
  - d. Leziuni traheale
  - e. Afectarea nervului recurent
  correct:
  - c. Embolie grasă
  Răspuns corect:  C

- question: 'Ce afirmaţii referitor la neoplaziile endocrine multiple sunt incorecte?'
  answers:
  - a. Sunt sindroame în care la acelaşi individ sunt prezente tumori interesând două sau mai multe glande endocrine
  - b. Ambele forme MEN I şi MEN II au şi o componentă tiroidiană
  - d. Sindromul MEN II prezintă trei varietăţi:tipul а, B şi tip familial
  correct:
  - c. În formele MEN I şi MEN II lipseşte componenta tiroidiană
  - e. Feocromocitoma este componentă a sindromului MEN I
  Răspuns corect:  C, E

- question: 'Sindromul Zollinger-Ellison are următoarea triadă caracteristică:'
  answers:
  - b. Hiposecreţie gastrică cantitativă şi calitativă
  - e. Valorile gastrinei serice sunt mult scăzute
  correct:
  - a. Ulcere multiple sau multiplu recidivante, atipică, cu evoluţie gravă şi rapidă
  - c. Chimismul gastric indică hipersecreţie bazală abudentă
  - d. Tumoră gastrino-secretoare (gastrinom)
  Răspuns corect:  A, C, D

- question: 'Ce afirmaţie referitor la feocromocitom este incorectă:'
  answers:
  - a. Boala reprezintă 1-5% din maladiile suprarenalelor
  - b. Are o incidenţă maximă între 20-35 de ani
  - c. Este o tumoră a celulelor cromofine
  - d. Mai frecvent se localisează în suprarenală dreaptă
  correct:
  - e. Este o tumoră malignă
  Răspuns corect:  E

- question: 'Care din afirmaţii referitoare la tratamentul feocromocitomului este incorectă:'
  answers:
  - a. Extirparea tumorii este unica soluţie terapeutică
  - b. Gestul chirurgical trebuie precedat, însoţit şi urmat de tratamentul medicamentos de reglare a tensiunii arteriale
  - c. Se practică enuclearea tumorii, rezecţia parţială a suprarenalelor sau adrenalectomia totală
  - d. Manipularea intraoperatorie a tumorii poate determină o criză hipertensivă
  correct:
  - e. Gestul chirurgical se efectuiază numai pe calea toracică
  Răspuns corect:  E

- question: 'Caracteristic timomului este:'
  answers:
  - b. Este o tumoră a suprarenalelor
  - d. Tumora poate fi asimptomatică, cu semne clinice compresive sau cu sindroame concomitente
  correct:
  - a. Este o tumoră a timusului
  - c. Este o tumoră a tiroidei
  - e. Miastenia în combinaţie cu timom are un prognostic nefaborabil
  Răspuns corect:  A, C, E

- question: 'Care din afirmaţii referitor la răsunetul clinic al miasteniei, la bolnavii cu timoame, este incorectă:'
  answers:
  - a. Debut acut şi decurgere malignă
  - b. Crize colinergice şa/sau miastenice
  - c. Dereglări severe motorice
  - d. Hipotrofii musculare
  correct:
  - e. Respiraţie parodoxală
  Răspuns corect:  E

- question: 'Următoarele fac parte din sindromul MEN-IIA:'
  answers:
  - e. Hiperplazie timică
  correct:
  - a. Carcinom medular multicentric
  - b. Feocromocitom
  - c. Hiperparatiroidism primar
  - d. Hiperplazie de celule C
  Răspuns corect:  A, B, C, D

- question: 'Următoarele fac parte din sindromul MEN-IIB, cu excepţia:'
  answers:
  - a. Carcinom medular de tiroidă
  - b. Feocromocitom
  - c. Ganglioneuroame intestinale
  - e. Nervoame mucoase
  correct:
  - d. Insulinom
  Răspuns corect:  D

- question: 'Următoarele semne clinice pledează pentru un hiperparatiroidism primar:'
  answers:
  - b. Crampe, fibrilaţii musculare.
  - e. Tegumente îngroşate, depigmentate.
  correct:
  - a. Sete, poliurie, pierdere ponderală.
  - c. Slăbiciune generală şi musculară severă.
  - d. Dureri în oasele supuse presiunii, fracturi patologice.
  Răspuns corect:  A, C, D

- question: 'Caracteristic pentru o criză tireotoxică este:'
  answers:
  - b. Termofobie, transpiraţii, parastezii
  - d. Exoftalmie, insomnie, atrofii musculare
  - e. Enoftalm
  correct:
  - a. Hipertermie, tahicardie, hipertensiune
  - c. Agitaţie, stare confuzională, vîrsaturi
  Răspuns corect:  A, C

- question: 'Pentru neoplazia endocrină multiplă de tip I (MEN1) nu este caracteristic:'
  answers:
  - a. Este compusă din tumori sau hiperplazii ale paratiroidelor, insulelor pancreatice şi hipofizei
  - c. În cadrul MEN 1 hiperparatiroidismul este cea mai comună manifestare (90-100%)
  - d. Adenomul hipofizar pare a fi prezent la aproape toţi pacienţii cu MEN 1
  - e. MEN 1 este o anomalie genetică autozomal dominantă
  correct:
  - b. Este compusă din tumori sau hiperplazii ale suprarenalelor, pancreasului şi hipofizei
  Răspuns corect:  B

- question: 'Ce afirmaţii referitor la tiroidita subacută de Quervain sunt adevărate?'
  answers:
  - a. Este mai frecventă la bărbaţi
  - e. Este o forma gravă de tireotoxicoză
  correct:
  - b. Apare după un epizod de infecţie a căilor respiratorii superioare
  - c. Debutează cu ferbă/frison
  - d. Tiroida creşte rapid în volum, dureroasă la palpare
  Răspuns corect:  B, C, D

- question: 'Sindromul MEN-IIA nu include:'
  answers:
  - a. Carcinom medular multicentric
  - b. Feocromacitom
  - c. Hiperparatiroidizm primar
  - d. Hiperplazie de celule C
  correct:
  - e. Hiperplazie timica
  Răspuns corect:  E



# AFECŢIUNILE  PLEURO-PULMONARE  SUPURATIVE
# (Abcesul pulmonar, Boala bronşiectatică, Gangrenul pulmonară, Pleureziile purulente nespecifice)

- question: 'Care este localizarea mai frecvente a abceselor pulmonare prin aspiraţie:'
  answers:
  - a. segmentele bazale anterioare ale lobilor inferiori
  - b. segmentele laterale ale lobilor inferiori
  - c. segmentele apicale ale lobilor superiori
  - d. segmentele anterioare ale lobilor superiori
  correct:
  - e. segmentele posterioare ale plămânilor   /preponderent lobii superior/
  Răspuns corect:  E

- question: 'Abcesele de origine bronhogenică în majoritatea cazurilor sunt:'
  answers:
  - a. multiple
  - b. periferice
  - e. bilaterale
  correct:
  - c. solitare
  - d. centrale
  Răspuns corect:  C, D

- question: 'Care simptom nu este caracteristic pentru I faza a abcesului pulmonar acut?'
  answers:
  - a. tuşea
  - b. sindromul de intoxicatie septico-purulenta
  - d. dispnee
  - e. sindromul algic
  correct:
  - c. expectoraţie purulentă
  Răspuns corect:  C

- question: 'Care metoda de diagnostic este mai informativa in faza I a abcesului pulmonar acut ?'
  answers:
  - a. scanarea cu radionuclizi
  - b. bronhoscopia
  - d. ultrasonografia
  - e. bronhografia
  correct:
  - c. tomografia asociata cu puncţie transtoracală
  Răspuns corect:  C

- question: 'Cea mai informativă metodă de diagnostic în faza a II a abcesului pulmonar acut este:'
  answers:
  - a. angiopulmonografia
  - c. scanarea cu radionuclizi
  - e. ultrasonografia
  correct:
  - b. radiografia de ansamblu în doua proiecţii
  - d. tomografia
  Răspuns corect:  B, D

- question: 'Cu care dintre formaţiunile cavitare ale toracelui diagnosticul  diferenţial al abcesului pulmonar este mai dificil de efectuat?'
  answers:
  - a. cancerul pulmonar forma atipică
  - b. tuberculoza pulmonara forma excavată
  - d. boala bronşiectatică
  - e. abces mediastinal
  correct:
  - c. chist supurat cu fistulă bronşică
  Răspuns corect:  C

- question: 'Hemoragia profuză survine mai frecvent în:'
  answers:
  - a. boala bronşiectatică
  - b. abcesele pulmonare
  - c. tuberculoza pulmonară
  - d. cancerul pulmonar
  correct:
  - e. gangrena pulmonară
  Răspuns corect:  E

- question: 'Metoda de elecţie în asanarea  abceselor voluminoase cu sechestre şi intoxicaţie septică pronunţată este:'
  answers:
  - a. toracopneumocinteza cu drenaj
  - b. bronhoscopia
  - c. lavajul bronsic prin cateterism
  - d. microtraheostomia
  correct:
  - e. toracopneumotomia şi drenarea abcesului
  Răspuns corect:  E


- question: 'Metoda de baza al tratamentului local în cazul rupturei abcesului piogen sau gangrenos în cavitatea pleurala, este:'
  answers:
  - a. bronhoscopia cu lavaj
  - b. puncţia pleurala repetată
  - c. pleurotomie minimă (închisă)
  - e. rezecţia pulmonară
  correct:
  - d. toracotomie largă, asanarea, drenarea cavitaţii pleurale şi abcesului
  Răspuns corect:  D

- question: 'Care din cauzele enumerate duc la cronicizarea abcesului pulmonar?'
  answers:
  - c. localizarea apicală a abcesului
  correct:
  - a. tratamentul consevator întârziat
  - b. terapia antibacteriană defectuoasă
  - d. terapia de detoxifiere neadecvată
  - e. abcesul pulmonar voluminos cu sechestre, asanat defectuos
  Răspuns corect:  A, B, D, E

- question: 'Complicaţia cea mai gravă a abcesului pulmonar cronic este:'
  answers:
  - a. tuberculizarea cavităţii reziduale
  - b. suprainfectarea cu fungi
  - d. hemoptizia
  - e. pioscleroza cu bronşiectazii secundare
  correct:
  - c. amiloidoza viscerală
  Răspuns corect:  C

- question: 'Metoda de elecţie în tratamentul abcesului pulmonar cronic este:'
  answers:
  - a. tratamentul medical general
  - b. bronhoscopie cu lavaj
  - c. toracopneumotomia cu drenaj
  - d. terapia antibacteriană
  correct:
  - e. rezecţia pulmonară
  Răspuns corect:  E

- question: 'Boala bronşiectatică se dezvolta pe fondul:'
  answers:
  - a. piosclerosei
  - b. pneumoniei cronice
  - d. emfizemului pulmonar
  - e. empiemului pleural cronic
  correct:
  - c. parenchimului neschimbat
  Răspuns corect:  C

- question: 'Care este simptomul funcţional dominant al bolii bronşiectatice?'
  answers:
  - a. sindromul algic
  - b. sindromul hemoptoic
  - c. sindromul de intoxicaţie septico-purulentă
  - e. febra
  correct:
  - d. sindromul bronşic
  Răspuns corect:  D

- question: 'Boala bronşiectatică afectează preponderent:'
  answers:
  - a. lobul superior al plămânului drept
  - b. lobul mediu al plămânului drept
  - c. lobul superior al plămânului stâng
  - d. lobul superior şi mediu al plămânului drept
  correct:
  - e. lobii inferiori
  Răspuns corect:  E

- question: 'Diagnosticul diferenţial al bolii bronşiectatice este  cel  mai  dificil  de efectuat cu una din următoarele maladii:'
  answers:
  - a. bronşitele supurate fără dilataţii bronşice
  - c. tuberculoza pulmonară
  - d. bronhocelele
  correct:
  - b. supuraţii bronhopulmonare cronice cu bronsiectazii secundare
  - e. formele chistice sau pseudochistice ale bolilor congenitale pulmonare
  Răspuns corect:  B, E

- question: 'Metoda cea mai eficace de diagnostic a bolii  bronşiectatice este:'
  answers:
  - a. angiopulmonografia
  - b. radiografia de ansamblu
  - c. tomografia
  - e. bronhoscopia
  correct:
  - d. bronhografia
  Răspuns corect:  D

- question: 'Tratamentul strict conservator a bolii bronşiectatice  este indicat în următoarele cazuri, exceptând:'
  answers:
  - a. bolnavii cu boli concomitente decompensate
  - b. stadiile iniţiale a maladiei, mai cu seama, la copii
  - c. cazurile de afectare a unui plămân şi al unui  lob sau mai mulţi din partea opusă
  - e. degerescenţa amiloida gravă a organelor interne
  correct:
  - d. afectarea limitată a ambilor plămâni
  Răspuns corect:  D

- question: 'Tratamentul conservator al bolii bronşiectatice include obligatoriu următoarele masuri, exceptând:'
  answers:
  - a. corecţia homeostazei
  - b. terapia de detoxicare
  - c. dezinfecţia bronşiilor şi a căilor aeriene superioare
  - e. tratamentul balneo-climatic
  correct:
  - d. combaterea secreţiei gastrice
  Răspuns corect:  D

- question: 'Tratamentul chirurgical al bolii bronşiectatice în dependenţă de procesul local  este indicat rar în următoarele cazuri:'
  answers:
  - a. bronşiectazii supurate unilaterale
  - b. plămân bronşiectatic complet distrus
  - c. bronşiectazii localizate bilateral bine delimitate
  - e. bronşiectazii secundare, mai cu seama, hemoptoice
  correct:
  - d. bronşiectazii difuze din copilărie, mai cu seamă bilaterale, fără complicaţie
  Răspuns corect:  D

- question: 'Care din complicaţiile postoperatorii imediate în rezecţiile pulmonare pentru boala  bronşiectatica  necesită  reintervenţie chirurgicala de urgenţă majoră?'
  answers:
  - a. atelectazia
  - b. şocul de durere
  - d. pneumatoraxul
  - e. inundaţia traheo-bronşică
  correct:
  - c. hemoragia intrapleurală
  Răspuns corect:  C

- question: 'Care metoda electiva de diagnostic în pleureziile purulente acute ?'
  answers:
  - a. ultrasonografia
  - b. scanarea plămânilor cu radionuclizi
  - d. angiopulmonografia
  - e. bronhoscopia
  correct:
  - c. radiografia de ansamblu în diferite poziţii   /de profil, de faţă, laterografia/
  Răspuns corect:  C


# MALADIILE NESUPURATIVE PLEURO-PULMONARE

- question: 'Care este calea principală de infestare în hidatidoza pulmonară primară:'
  answers:
  - a. Calea respiratorie
  - c. Calea directă prin mucoase
  - d. Calea directă prin muşcături
  - e. Calea directă prin tegumente
  correct:
  - b. Calea digestivă
  Răspuns corect:  B

- question: 'Echinococoză pulmonară secundară se produce pe:'
  answers:
  - a. Cale enterogenă
  - c. Prin mucoase
  - e. Pe cale limfogenă
  correct:
  - b. Cale bronhogenă
  - d. Pe cale hematogenă
  Răspuns corect:  B, D

- question: 'Care dintre cele enumerate reprezintă elementele structurale proprie ale chistului hidatic pulmonar, exceptând:'
  answers:
  - a. Membrana chitinoasă
  - c. Membrana proligeră
  - d. Scolecş
  - e. Vezicule fiice
  correct:
  - b. Adventiţia (advenţia)
  Răspuns corect:  B

- question: 'Care din complicaţii chistului hidatic pulmonar se asociază cu şoc anafilactic:'
  answers:
  - a. Ruperea în bronhie
  - b. Supuraţia
  - d. Ruperea în bronhie cu aspiraţie în plămânul sănătos
  - e. Hemoragia
  correct:
  - c. Ruperea în cavitatea pleurală
  Răspuns corect:  C

- question: 'Care din cele enumerate este complicaţia cea mai frecventa al chisturilor aeriene pulmonare subpleurale?'
  answers:
  - a. Supuraţia
  - c. Hemoptezia
  - d. Cancerizarea
  - e. Sindromul compresiv
  correct:
  - b. Pneumotoraxul spontan
  Răspuns corect:  B

- question: 'Diagnosticul diferenţial al chisturilor aeriene pulmonare se face cu următoarele afecţiuni, exceptând:'
  answers:
  - a. Tuberculoza excavată
  - b. Cancerul cavitar
  - d. Abcesul piogen pulmonar
  - e. Pleurezia purulentă interlobară cu fistula bronşica
  correct:
  - c. Hidatidoza pulmonară necomplicată
  Răspuns corect:  C

- question: 'Care din investigaţiile enumerate sunt primordiale în stabilirea diagnosticului de chist aerian pulmonar:'
  answers:
  - a. Investigaţie cu radionuclizi
  - b. Toracoscopia
  - d. Bronhoscopia
  - e. Investigaţiile citologice şi morfologice
  correct:
  - c. Investigaţii radiologice
  Răspuns corect:  C

- question: 'Factorul etiologic al revărsatului pleural nesupurativ este, cu excepţia:'
  answers:
  - a. Trauma canalului limfatic toracic (chilotoraxul)
  - b. Traumatismul toracelui cu lezarea pleurei, arterelor intercostale, parenchimului pulmonar (hemotoraxul)
  - c. Revărsatul  plevral pancreatogen
  - e. Revărsatul pleural malign (metastatic)
  correct:
  - d. Ruperea abcesului pulmonar în cavitatea pleurală
  Răspuns corect:  D

- question: 'Care din semnele enumerate sunt caracteristice pentru pneumotoraxul cu supapa, cu excepţie:'
  answers:
  - a. Dureri violente
  - b. Dispnee pronunţată
  - c. Bombarea unui semitorace
  - d. Turgistenţa venelor gâtului şi a  membrilor superioare
  correct:
  - e. Hemoptizii
  Răspuns corect:  E

- question: 'Semnele radiologice caracteristice pentru pneumotoraxul compresiv sunt, cu excepţia:'
  answers:
  - a. Hipertransparenţa unui hemitorace
  - b. Deplasarea pronunţata a mediastinului
  - c. Colabarea plămânului
  - d. Deplasarea semidiafragmului corespunzător spre cavitatea peritoneală
  correct:
  - e. Balansarea mediastinului
  Răspuns corect:  E

- question: 'Care este procedeul terapeutic primordial în ajutorul medical de urgenţa majoră în pneumotoraxul cu supaăa?'
  answers:
  - a. Administrarea de oxigen
  - b. Administrarea analepticilor respiratori
  - d. Administrarea  de cardiotomiei
  - e. Respiraţie controlată
  correct:
  - c. Transferarea pneumotoraxul cu subapă în pneumotoraxul deschis
  Răspuns corect:  C


# CARDIOPATIILE  CONGENITALE  ŞI  DOBÂNDITE

- question: 'Canalul arterial permiabil (CAP),  selectaţi răspunsul fals:'
  answers:
  - a. CAP uneşte cârja aortei cu artera pulmonară
  - c. în perioada iniţială a CAP oxigenarea sângelui în circuitul mic este mărită
  - d. iniţial în hipertenzia pulmonară miofibrele arteriolare se hipertrofiază
  - e. în faza IV a hipertenziei pulmonare torentul sanguin  îşi schimbă direcţia prin CAP
  correct:
  - b. CAP uneşte aorta ascendentă cu artera pulmonară
  Răspuns corect:  B

- question: 'Defectul septului ventricular (DSV), care răspuns este greşit?'
  answers:
  - b. viciul din palid se poate transforma în cianotic
  correct:
  - a. în DSV suflul sistolic se propagă pe toată cutia toracică
  - c. operaţia paliativă poate fi efectuată numai după vârsta de un an
  - d. operaţia paliativă poate fi efectuată în primele trei luni de viaţă
  - e. operaţia efectuată în faza cianogenă este neefectivă
  Răspuns corect:  A, C, D, E

- question: 'Defectul septului atrial (DSA), care este răspunsul incorect ?'
  answers:
  - a. în plastia defectului septului se aplică surjet în cotinuu
  - c. electrocardiografia  depistează semne de suprasolicitare numai a camerelor drepte ale inimii
  - d. ventriculul stâng rămâne subsolicitat
  - e. schimbările morfologice în micul circuit sangvin apar treptat
  correct:
  - b. în plastia DSA se aplică suturi punctate
  Răspuns corect:  B

- question: 'Coarctaţia aortei, care răspuns este fals?'
  answers:
  - a. îngustarea aortei are loc după începutul arterei  brahiocefalice
  - b. îngustarea are loc distal de artera subclavă stângă
  - c. sângele trece în partea inferioară a corpului prin  artera subclavă dilatată
  - d. tensiunea în arterele femorale este micşorată
  correct:
  - e. plastia în coarctaţia aortei constă în excizia segmentului îngustat cu suturarea termino-terminal sau şutare cu proteză vasculară, sau aplicarea protezei vasculare
  Răspuns corect:  E

- question: 'Stenoza izolată a arterei pulmonare (SIAP), care răspuns este fals?'
  answers:
  - a. în SIAP îngustarea poate fi valvulară sau subvalvulară
  - b. în SIAP trunchiul arterei pulmonare este dilatat
  - c. în SIAP tensiunea în micul circuit sangvin reflector se măreşte
  - d. în SIAP zgomotul doi pe artera pulmonară este diminuat
  correct:
  - e. operaţia în SIAP consta în comisurotomie deschisa, excizia surplusului muscular, aplicarea    peticului din material plastic
  Răspuns corect:  E

- question: 'Tratamentul chirurgical în tetrada Fallot. Care răspuns este fals?'
  answers:
  - a. operaţia radicală subînţelege dilatarea arterei pulmonare, plastia defectului septal cu deplasarea aortei spre ventriculul stâng
  - b. operaţiile paliative sunt mai efective pînă la vârsta de trei ani
  - c. mai frecvent în operaţia paliativă se restituie anastomoza a.subclavă - ramura arterei pulmonare pe dreapta
  - e. la ECG predomină semne de suprasolicitare a ventricului drept
  correct:
  - d. în tetrada Fallot numărul eritrocitelor este diminuat
  Răspuns corect:  D

- question: 'Tamponada cardiacă se traduce prin următoarele semne:'
  answers:
  - a. presiune venoasă crescută
  - c. minut-volum cardiac scăzut
  - d. umplere diastolică a inimii crescută
  - e. tensiune arterială scăzută
  correct:
  - b. şoc
  Răspuns corect:  B

- question: 'Anevrismul disecant al aortei toracice este favorizat, în primul rând, de:'
  answers:
  - a. ateroscleroză
  - b. sifilis
  - c. degenerescenta tunicii medii
  - d. traumă
  correct:
  - e. coarctaţia de aortă
  Răspuns corect:  E

- question: 'Stenoza mitrală poate rezulta din următoarele:'
  answers:
  - d. creştere a rezistenţei periferice
  correct:
  - a. scădere a debitului cardiac
  - b. fibrilaţie arterială
  - c. embolie sistemică
  - e. edem pulmonar
  Răspuns corect:  A, B, C, E

- question: 'Cel mai frecvent viciu cardiac congenital este:'
  answers:
  - a. tetrada Fallot
  - b. comunicarea interventriculară
  - c. canalul arterial persistent
  - e. transpoziţia vaselor mari
  correct:
  - d. comunicarea interatrială
  Răspuns corect:  D


# ARTERIOPATIILE PERIFERICE OBLITERANTE

- question: 'Tratamentul fistulelor artero-venoase consta în:'
  answers:
  - b. ligaturarea arterei distale de fistula
  - c. amputaţia membrului
  - d. ligaturarea venei distale de fistula
  - e. tratament exclusiv medicamentos
  correct:
  - a. excizia fistulei cu restabilirea continuităţii  vaselor
  Răspuns corect:  A

- question: 'Schimbările trofice condiţionate de ischemia cronica  includ următoarele:'
  answers:
  - e. osteoporoza
  correct:
  - a. căderea parului
  - b. modificările unghiilor
  - c. atrofia pielei
  - d. atrofia musculaturii
  Răspuns corect:  A, B, C, D

- question: 'Embolii arteriali se fixează mai des la nivelul:'
  answers:
  - a. bifurcaţiei aortice
  - b. arterei iliace comune
  - d. vaselor cerebrale
  - e. arterei poplitee
  correct:
  - c. arterei femurale comune
  Răspuns corect:  C

- question: 'Ce afirmaţie referitoare la particularităţile maladiei Burger este incorecta?'
  answers:
  - a. se întâlneşte mai frecvent la bărbaţi
  - b. se observa intre 20 şi 40 ani
  - c. este însoţita de claudicatie intermitenta
  - d. este în raport strâns cu fumatul
  correct:
  - e. prezintă o forma deosebita de ateroscleroza
  Răspuns corect:  E

- question: 'Claudicatia intermitenta ca urmare a aterosclerozei avansate a arterelor iliace externe este mai efectiv tratata prin'
  answers:
  - a. simpatectomie lombara
  - b. gimnastica efectuata zilnic
  - d. vasodilatatoare
  - e. rezecţie a segmentului afectat cu anastomoza
  correct:
  - c. by-pass
  Răspuns corect:  C

- question: 'La apariţia trombozei acute arteriale  al membrilor contribuie:'
  answers:
  - a. cardioscleroza aterosclerotica
  - b. endocardita septica
  - c. viciul mitral reumatic
  - e. toate cele enumerate
  correct:
  - d. ateroscleroza obliterantă al vaselor membrilor inferioare
  Răspuns corect:  D

- question: 'Date despre starea arterelor magistrale distal de locul emboliei putem obţine la:'
  answers:
  - a. capilaroscopie
  - b. examenul obiectiv
  - c. examinare radionuclida
  - e. toate cele menţionate
  correct:
  - d. dopplerografie
  Răspuns corect:  D

- question: 'Operaţia de baza, efectuată în embolia arterii femurale este:'
  answers:
  - a. dilatare renghenoendovasculara
  - b. trombintimectomia
  - d. profundoplastia
  - e. by-pass
  correct:
  - c. embolectomia
  Răspuns corect:  C

- question: 'Factorul principal, ce determină tactica de tratament în insuficienţa arterială acuta este:'
  answers:
  - a. vârsta pacientului
  - b. gravitatea stării generale
  - d. prezenţa ocluziei aterosclerotice concomitente
  - e. localizarea trombului sau embolului
  correct:
  - c. gradul ischemiei membrului
  Răspuns corect:  C

- question: 'Restabilirea circulaţiei sanguine în insuficienţa arteriala acuta al membrului este absolut contraindicata în:'
  answers:
  - a. ischemia de gr. Ia lent progresiva
  - b. ischemia  de gr. Ilb pacientul cu infarct miocardic
  - c. ischemia de gr. IIIa la pacient cu ictus
  - e. vârsta pacientului 85 ani
  correct:
  - d. ischemia de gr. IIIc
  Răspuns corect:  D

- question: 'Amputaţia ca operaţie de elecţie în insuficienţa arterială acuta al membrelor este indicată:'
  answers:
  - a. în ateroscleroza avansată  al vaselor magistrale al membrelor cu manifestările incipiente ale trombozei acute
  - b. la pacienţi în vârsta avansată
  - c. în gradul IIIb de ischemie
  - e. în internare tardiva a pacientului
  correct:
  - d. numai în gradul IIIc de ischemie
  Răspuns corect:  D

- question: 'Terapia conservativa în embolie acută arterială poate fi metoda de elecţie în :'
  answers:
  - a. în stadiu avansat al ischemiei acute
  - b. la pacienţi în vârsta 80 ani
  - c. la pacienţi cu patologie concomitentă gravă
  - e. în prezenţa maladiilor trombogene şi embologene
  correct:
  - d. în caz de contraindicaţii absolute pentru tratament chirurgical
  Răspuns corect:  D

- question: 'Pentru tromboza acuta a arterei magistrale a piciorului  nu este caracteristic:'
  answers:
  - a. paliditate
  - b. dureri
  - d. parastezie
  - e. disparitia pulsului
  correct:
  - c. edem
  Răspuns corect:  C

- question: 'Către sindromul ischemic a membrelor inferioare se atirna tot în  afara de:'
  answers:
  - a. lipsa pulsului
  - b. parastezii
  - d. paralizie inferioara
  - e. paliditatea tegumentelor si dureri
  correct:
  - c. ulcere trofice a gambei
  Răspuns corect:  C

- question: 'In embolia arterei femurale, ischemie gradul III  (contractura piciorului) metoda de electie este:'
  answers:
  - a. embolectomie de urgenta
  - b. terapie trombolitica
  - c. terapie cu anticoagulati
  - d. terapie simptomatica
  correct:
  - e. amputaţia primara a piciorului
  Răspuns corect:  E

- question: 'Embolia arterei brahiale poate fi cauzata de ori  şi care maladie cu excepţie:'
  answers:
  - a. stenoza mitrala
  - b. infarct miocardic acut
  - c. anevrisma cordului
  - e. comprimarea arterei subclavia cu o coasta  cervicala adaugatoare
  correct:
  - d. anevrism a aortei abdominale
  Răspuns corect:  D

- question: 'Viabilitatea piciorului ischemizat de origina embolica   se poate aprecia prin:'
  answers:
  - a. sfigmografie
  - b. reovazografie
  - d. pletismografie
  - e. termografie
  correct:
  - c. scintigrafie cu Te 99 (tehnetii)-pirofosfat
  Răspuns corect:  C

- question: 'Cel mai perfect instrument pentru embolectomie din artera magistrala este:'
  answers:
  - a. inelul vascular Wolmar
  - b. vacuum-aspirator
  - d. cateter Dormia
  - e. pensa fenestrata
  correct:
  - c. cateterul Fogarti cu balonaş
  Răspuns corect:  C

- question: 'Anevrismele aterosclerotice periferice se localizează cel mai frecvent la nivelul arterei'
  answers:
  - a. carotide
  - b. subclaviculare
  - c. femurale
  - e. ulnare
  correct:
  - d. poplitee
  Răspuns corect:  D

- question: 'Un bolnav în vârsta de 80 ani se prezintă cu maladie cardiaca severa si claudicatie intermitenta pronunţata in piciorul drept. Tratamentul de electie in acest   caz este:'
  answers:
  - a. by-pasul aorto-femural
  - b. by-pasul axilo-femural drept
  - d. by-pasul aorto-iliac drept
  - e. simpatectomia dreapta
  correct:
  - c. by-pasul femuro-femural
  Răspuns corect:  C

- question: 'Sindromul Leriş - aceasta este:'
  answers:
  - a. artereita nespecifică brahiocefala
  - c. capilaropatia periferica a extremităţilor
  - d. trombangiita migranta
  - e. ocluzia venei cave inferioare
  correct:
  - b. ocluzie aterosclerotica a bifurcatiei aortei
  Răspuns corect:  B

- question: 'Pentru sindromul Leriş este caracteristic, tot afara de:'
  answers:
  - a. hipotermia membrelor inferioare
  - b. paliditatea pielei
  - d. dureri in regiunea fesiera la şezut
  - e. impotentei
  correct:
  - c. tromboflebita migranta a membrelor
  Răspuns corect:  C

- question: 'Pentru sindromul Takayasu nu este caracteristic:'
  answers:
  - a. dispariţia pulsului la membrele inferioare
  - c. grafospasm
  - d. cefalee, vertijuri
  - e. scădere vederii până la cecitate
  correct:
  - b. dispariţia pulsului membrele inferioare
  Răspuns corect:  B

- question: 'Pentru tratamentul chirurgical in sindromul Leriş  ce-a mai reuşita metoda va fi:'
  answers:
  - a. simpatectomie lombara
  - b. simpatectomie periarteriala
  - d. sunt aorto-femural unilateral
  - e. trombectomie cu cateterul Fogarti
  correct:
  - c. sunt aorto-femural bifurcaţi anal
  Răspuns corect:  C

- question: '.Alegeţi afirmaţie corectă  referitor la manifestarea clinica aneurizmului aortic abdominal (AAA):'
  answers:
  - a. În 70-75% de cazuri din toate aneurizmului aortic abdominal  sunt simptomatice la diagnosticare
  - c. AAA frecvent se diagnostica întâmplător în timpul operaţiilor abdomenale pentru alte patologii
  - d. Pacienţii des pun diagnosticul de sinestător
  - e. Hipotenzie şi şoc sunt caracteristice pentru aneurizmului aortic abdominal neerupt
  correct:
  - b. Examinare fizicala de rutină şi metodele imagistice  permit a stabili diagnosticul
  Răspuns corect:  B

- question: 'Alegeţi afirmaţiile corecte referitor la manifestarea clinica aneurizmului aortic abdominal erupte:'
  answers:
  - e. Icter
  correct:
  - a. Dureri abdomenale, şoc şi tumora pulsativa abdominală
  - b. Dureri sunt avansate, dar sunt caracteristice şi pentru aneurizmului aortic abdominal neerupt
  - c. Dureri abdomenale nu depind de poziţia bolnavului şi relativ sunt constante
  - d. Hipotenzie şi şoc nu sunt caracteristice pentru aneurizmului aortic abdominal neerupt
  Răspuns corect:  A, B, C, D

- question: 'Alegeţi afirmaţiile corecte referitor la aneurizmului aortic abdominal:'
  answers:
  - d. Majoritatea aneurizmului aortic abdominal sunt simptomatice înainte de erupţie
  - e. Aneurizmului aortic abdominal este asociat cu boala varicoasă a membriilor inferioare
  correct:
  - a. Creşterea incidenţei a aneurizmului aortic abdominal reflecta creştere vârstei medie a populaţiei, îmbunătăţirea metodelor diagnostice
  - b. Incidenţa aneurizmului aortic abdominal creşte cu alte leziuni aterosclerotice
  - c. La pacienţi cu aneurizmul femoral sau popliteal incidenţa aneurizmului aortic abdominal creşte până la 50%
  Răspuns corect:  A, B, C


# AFECŢIUNILE CHIRURGICALE A DIAFRAGMULUI

- question: 'Enumeraţi părţile slabe ale diafragmului:'
  answers:
  - d. Triunghului lui Paimerov
  correct:
  - a. Fisura Larrey
  - b. Fisura Morgani
  - c. Fisura Bohdalec
  - e. Hiatus aorticus
  Răspuns corect:  A, B, C, E

- question: 'Tabloul clinic al herniei hiatale prin alunecare include :'
  answers:
  - a. Disfagie
  - b. Vome frecvente
  - d. Micşorarea ponderei corporeale
  - e. Nici un răspuns corect
  correct:
  - c. Perozis
  Răspuns corect:  C

- question: 'Pacientul a fost internat in clinică cu dureri în regiunea hemitoracelui stăng, dispnee care se intensifică postprandial şi la efort fizic,de asemenea în poziţie orizontală a corpului, greţuri,vome repetate care uşurează starea pacientului. La radiografia cutiei toracice sa depistat  o formaţiune de volum cu nivel hidroaeric. Stabiliţi  diagnosticul :'
  answers:
  - a. Pneumonie abscedantă pe stânga
  - b. Angina pectorală
  - d. Hemotorace pe stânga
  - e. Hernia posttraumatică a diafragmului
  correct:
  - c. Hernie hiatală prin alunecare
  Răspuns corect:  C

- question: 'Care sunt condiţiile necesare pentru diagnosticul radiologic al herniilor hiatale prin alunecare?'
  answers:
  - a. În poziţie verticală
  - b. In pozitie semisezinda
  - d. Hipotonie artificială a duodenului
  - e. În poziţie orizontală
  correct:
  - c. În poziţie Trendelenburg
  Răspuns corect:  C

- question: 'Enumeraţi cele mai frecvente semne clinice ale herniei hiatale prin alunecare:'
  answers:
  - b. Dureri retrosternale; Melenă; Ocluzie intestinală; Vome
  - c. Ocluzie intestinală; Vome; Perozis
  - d. Perozis
  - e. Perozis; Vome; Melenă
  correct:
  - a. Perozis; Dureri retrosternale; Melenă
  Răspuns corect:  A
`
